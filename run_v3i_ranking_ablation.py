...
# ============================================================
# V3G4 EXIT POLICY ? MUST LOAD BEFORE OUTPUT PATH CONSTANTS
# ============================================================

V3G4_EXIT_POLICY = os.environ.get(
    "V3G4_EXIT_POLICY",
    "BASELINE",
).upper()

VALID_V3G4_EXIT_POLICIES = {
    "BASELINE",
    "EMA10",
}

if V3G4_EXIT_POLICY not in VALID_V3G4_EXIT_POLICIES:
    raise ValueError(
        "Invalid V3G4_EXIT_POLICY: "
        f"{V3G4_EXIT_POLICY}"
    )

V3I_RANK_POLICY = os.environ.get(
    "V3I_RANK_POLICY",
    "BASELINE",
).upper()

VALID_V3I_RANK_POLICIES = {
    "BASELINE",
    "NO_ACCEL_RANK",
}

if V3I_RANK_POLICY not in VALID_V3I_RANK_POLICIES:
    raise ValueError(
        f"Invalid V3I_RANK_POLICY: "
        f"{V3I_RANK_POLICY}"
    )

V3G4_OUTPUT_TAG = (
    f"{V3G4_EXIT_POLICY.lower()}_"
    f"{V3I_RANK_POLICY.lower()}"
)
...
import run_v3d_diagnostics as v3d_diagnostics

V3G4_WINDOW = os.environ.get(
    "V3G4_WINDOW",
    "2021",
).upper()
...
if V3G4_WINDOW not in ROBUSTNESS_WINDOWS:
    raise ValueError(
        f"Invalid V3G4_WINDOW: {V3G4_WINDOW}"
    )

ROBUSTNESS_START, ROBUSTNESS_END = (
    ROBUSTNESS_WINDOWS[V3G4_WINDOW]
)

V3G4_WINDOW_TAG = V3G4_WINDOW.lower()

# The data helpers read these module globals. This changes only the
# historical test window; all V3D3 risk and exit limits remain unchanged.
v3d_diagnostics.BACKTEST_START = ROBUSTNESS_START
v3d_diagnostics.BACKTEST_END = ROBUSTNESS_END
...
BACKTEST_START = ROBUSTNESS_START
BACKTEST_END = ROBUSTNESS_END
from strategies.pullback_candidate_v3d2 import (
    PullbackCandidate,
    evaluate_pullback_candidate,
)
from run_v3e2_signal_capture_diagnostics import (
    RouteSignal,
    build_new_routes,
...
# ============================================================================
# V3E6 SELECTIVE DEVELOPMENT â€” V3D3 RISK/EXIT ENGINE FROZEN
# ============================================================================
...
TRADE_HISTORY_FILE = Path(
    f"logs/backtests/v3g4_eth365_robustness_{V3G4_WINDOW_TAG}_{V3G4_OUTPUT_TAG}_trades.csv"
)
EVENT_HISTORY_FILE = Path(
    f"logs/backtests/v3g4_eth365_robustness_{V3G4_WINDOW_TAG}_{V3G4_OUTPUT_TAG}_events.csv"
)
SUMMARY_FILE = Path(
    f"reports/v3g4_eth365_robustness_{V3G4_WINDOW_TAG}_{V3G4_OUTPUT_TAG}_summary.csv"
)
SYMBOL_SUMMARY_FILE = Path(
    f"reports/v3g4_eth365_robustness_{V3G4_WINDOW_TAG}_{V3G4_OUTPUT_TAG}_symbols.csv"
)
...
# Profit-layer churn control only. V3D3 risk/surveillance remains frozen.
ROUTE_SIGNAL_COOLDOWN_HOURS = 12

# V3G4 historical-robustness route.
# Signal timestamps/levels are regenerated independently
# from historical ETHUSDT hourly candles.
ETH365_ROUTE = "ETH_365D"

# Independent validation history.
# 366 days provides a complete prior 365-day hourly window
# before the first May 2026 validation decision.
ETH365_WARMUP_START = (
    ROBUSTNESS_START
...
def _bounded_stop(
    ...
    atr: float,
    proposed_stop: float,
    minimum_atr_distance: float,
    maximum_atr_distance: float,
) -> float:
    minimum_distance = atr * minimum_atr_distance
    maximum_distance = atr * maximum_atr_distance

    raw_distance = max(
        entry_price - proposed_stop,
        0.0,
    )

    bounded_distance = min(
    ...
    return max(
        0.0,
        entry_price - bounded_distance,
    )


def _route_quality_allowed(
    symbol: str,
    route: str,
    signal: RouteSignal,
...
def _convert_route_signal(
    ...
    candle = frame.loc[decision_time]
    position = frame.index.get_loc(decision_time)
    ...
    allowed = _route_quality_allowed(
        symbol=symbol,
        route=route,
        signal=signal,
    ...
    if route == "IGNITION":
        proposed_stop = max(
            recent_low_6h - atr * 0.20,
            ema20 - atr * 0.65,
            ema50 - atr * 0.25,
        )

        stop_price = _bounded_stop(
            entry_price=entry_price,
            atr=atr,
            proposed_stop=proposed_stop,
            minimum_atr_distance=0.90,
            maximum_atr_distance=2.20,
        )

        target_multiple = 2.20
        ...
        proposed_stop = max(
            breakout_support - atr * 0.60,
            ema20 - atr * 0.50,
            recent_low_6h - atr * 0.15,
        )
        ...
        stop_price = _bounded_stop(
            ...
            atr=atr,
            proposed_stop=proposed_stop,
            minimum_atr_distance=1.00,
            maximum_atr_distance=2.40,
        )

        target_multiple = 2.40
    ...
    return ProfitCandidate(
        ...
        reasons=(
            f"{route} signal",
            f"relative strength {relative_strength:.1f}",
        ),
    )



def inject_eth365_candidates(
    candidates_by_time: dict[
        pd.Timestamp,
        list[ProfitCandidate],
    ],
    frames_1h: dict[str, pd.DataFrame],
    common_1h: pd.DatetimeIndex,
) -> int:
    """
    Independently generate and inject ETH 365D breakout signals.

    IMPORTANT:
    - 365D level uses an independent 366-day hourly history fetch.
    - Initial stop reuses the existing V3E6 breakout stop structure.
    - No new leverage/risk rule is introduced.
    """
    ...
    raw = get_historical_candles(
        ...
        limit=1000,
        start_time=ETH365_WARMUP_START.strftime(
            "%Y-%m-%d %H:%M:%S"
        ),
        end_time=ROBUSTNESS_END.strftime(
            "%Y-%m-%d %H:%M:%S"
        ),
    ...
    if raw.empty:
        raise ValueError(
            "No ETHUSDT history for "
            "V3G4 365D validation."
        )
    ...
    # Previous 8,760 COMPLETED hourly highs only.
    # The current candle is excluded with shift(1).
    raw["PRIOR_365D_HIGH"] = (
        raw["high"]
        .shift(1)
        .rolling(
    ...
    # Fresh breakout cross only.
    raw["HIGH_365D_SIGNAL"] = (
        (
            raw["close"]
            > raw["PRIOR_365D_HIGH"]
        )
    ...
    print(
        "V3G4 independently generated "
        f"ETH365 signals: {len(signals)}"
    )

    frame = frames_1h["ETHUSDT"]
    common_times = set(common_1h)
    ...
    for row in signals.itertuples(
        ...
        if decision_time not in common_times:
            continue
        ...
        position_index = frame.index.get_loc(
            decision_time
        )
        ...
        # Reuse the existing V3E6 adaptive-breakout
        # structural stop architecture:
        #
        # breakout support
        # EMA20 support
        # recent 6h structure
        #
        # then bound distance between 1.0 and 2.4 ATR.
        proposed_stop = max(
            breakout_level - atr * 0.60,
            ema20 - atr * 0.50,
            recent_low_6h - atr * 0.15,
        )
        ...
        # 2.40R is only a projected R:R reference
        # required by the existing entry pre-flight.
        # It is NOT a take-profit exit.
        target_multiple = 2.40
    ...
    print(
        "V3G4 ETH365 robustness signals injected: "
        f"{injected}"
    )
...
def build_profit_candidates(
    pullbacks_by_time: dict[
        pd.Timestamp,
        list[PullbackCandidate],
    ],
    route_signals_by_time: dict[
        pd.Timestamp,
        dict[
            str,
            dict[
    ...
    for decision_time in common_1h:
        ...
        # V3E6: broad pullback route disabled after failing both
        ...
        # Keep only the highest-quality route for each asset at
        # the same decision time. Pullback wins ties because it
        # enters after an observed retracement.
        route_priority = {
            "PULLBACK": 3,
            "ADAPTIVE_BREAKOUT": 2,
            "IGNITION": 1,
...
def build_shock_tables(
    ...
    total_surveillance_steps = len(common_15m)

    for decision_index, decision_time in enumerate(
        common_15m,
        start=1,
    ):
        if (
            decision_index == 1
            or decision_index % 5000 == 0
            or decision_index == total_surveillance_steps
        ):
            print(
                "Surveillance progress: "
                f"{decision_index:,}/"
                f"{total_surveillance_steps:,}"
            )
...
def rank_valid_candidates(
    ...
    frames_1h: dict[str, pd.DataFrame],
    decision_time: pd.Timestamp,
) -> list[tuple[ProfitCandidate, float]]:
    ...
    table["route_rank"] = percentile_rank(table["route_quality"])

    if V3I_RANK_POLICY == "BASELINE":
        table["selection_score"] = (
            table["setup_rank"] * 0.35
            + table["rr_rank"] * 0.20
            + table["route_rank"] * 0.15
            + table["momentum_rank"] * 0.15
            + table["slope_rank"] * 0.10
            + table["adx_rank"] * 0.05
        )
    else:
        # V3I-A PRE-LOCKED ABLATION:
        # Remove slope and ADX ranking preference only.
        # No redistribution or reweighting.
        table["selection_score"] = (
            table["setup_rank"] * 0.35
            + table["rr_rank"] * 0.20
            + table["route_rank"] * 0.15
...
def run_backtest() -> dict[str, Any]:
    print("=" * 126)
    print("FA CRYPTO ENGINE - V3G4 ETH365 HISTORICAL ROBUSTNESS")
    print("=" * 126)
    print(
        f"Robustness window: {BACKTEST_START} to {BACKTEST_END}"
    )
    print(
        "Risk core: V3D3 frozen | "
        f"ETH365 exit policy: {V3G4_EXIT_POLICY}"
    )

    frames_15m, frames_1h, common_15m, common_1h = prepare_frames()

    # Causal completed-candle EMA10.
    # No future candle is used.
    eth_ema10_1h = (
        frames_1h["ETHUSDT"]["close"]
        .astype(float)
        .ewm(
            span=10,
            adjust=False,
    ...
    print("Building frozen pullback candidates...")
    pullbacks_by_time = build_candidates(
        frames_1h,
        common_1h,
        market_states,
        asset_shocks,
    ...
    print("Building controlled ignition and adaptive-breakout routes...")
    route_signals_by_time, route_inventory = build_new_routes(
        frames_1h,
        common_1h,
        market_states,
        asset_shocks,
    ...
    candidates_by_time = build_profit_candidates(
        pullbacks_by_time=pullbacks_by_time,
    ...
    def manage_15m_candle(event_time: pd.Timestamp) -> None:
        ...
        for symbol in list(positions.keys()):
            ...
            use_baseline_atr_trailing = not (
                position.route == ETH365_ROUTE
                and V3G4_EXIT_POLICY
                == "EMA10"
            )

            if (
                highest_r >= BREAKEVEN_TRIGGER_R
                and use_baseline_atr_trailing
            ):
    ...
    total_backtest_steps = len(common_15m)

    for event_index, event_time in enumerate(
        common_15m,
        start=1,
    ):
        if (
            event_index == 1
            or event_index % 5000 == 0
            or event_index == total_backtest_steps
        ):
            print(
                "Backtest progress: "
                f"{event_index:,}/"
                f"{total_backtest_steps:,}"
            )
        ...
        if V3G4_EXIT_POLICY == "EMA10":
            eth_position = positions.get(
                "ETHUSDT"
            )

            if (
                eth_position is not None
                and eth_position.route
                == ETH365_ROUTE
            ):
                eth_frame = frames_1h[
                    "ETHUSDT"
        ...
        for candidate, selection_score in ranked:
            ...
            # Standardized fee-aware route filters selected only where
            # development and validation results agreed.
            route_rr_allowed = (
                (
                    candidate.route == "IGNITION"
                    and symbol == "LINKUSDT"
                    and 2.00 <= actual_reward_risk <= 2.19
                )
                or (
                    candidate.route == "ADAPTIVE_BREAKOUT"
                    and symbol == "SOLUSDT"
                    and 2.20 <= actual_reward_risk <= 2.30
                )
                or (
                    candidate.route == ETH365_ROUTE
                    and symbol == "ETHUSDT"
                )
            )

            if not route_rr_allowed:
                skipped_risk_entries += 1
                continue
    ...
    print()
    print("=" * 126)
    print("FA CRYPTO ENGINE - V3G4 ETH365 ROBUSTNESS RESULT")
    print("=" * 126)
    print(f"Starting Capital             : â‚¬{STARTING_CAPITAL:.2f}")
    ...
    print(f"Expectancy Per Trade         : â‚¬{expectancy:.2f}")
    print(f"Maximum Drawdown             : {max_drawdown:.2f}%")
    print(f"Total Trading Fees           : â‚¬{total_fees:.2f}")
    ...
    for route in (
        ...
        print(
            f"{route:<22}: "
            f"Trades {route_counts[route]:>3} | "
            f"P&L â‚¬{route_profit[route]:>9.2f}"
        )
...
if __name__ == "__main__":
    try:
        ...
        print()
        print("V3E6 development stopped manually.")

    except Exception as error:
        print()
        print("=" * 126)
        print("V3E6 DEVELOPMENT ERROR")
        print("=" * 126)
