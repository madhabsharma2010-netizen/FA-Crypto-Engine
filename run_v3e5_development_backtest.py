...
# ============================================================================
# V3E5 DEVELOPMENT SETTINGS — V3D3 RISK/EXIT ENGINE FROZEN
# ============================================================================
...
TRADE_HISTORY_FILE = Path(
    "logs/backtests/v3e5_development_trade_history.csv"
)
EVENT_HISTORY_FILE = Path(
    "logs/backtests/v3e5_development_event_history.csv"
)
SUMMARY_FILE = Path(
    "reports/v3e5_development_backtest_summary.csv"
)
SYMBOL_SUMMARY_FILE = Path(
    "reports/v3e5_development_symbol_summary.csv"
)
...
def _route_quality_allowed(
    ...
    if route == "IGNITION":
        if symbol not in IGNITION_ENABLED:
            return False

        if symbol in {"BTCUSDT", "ETHUSDT"}:
            return signal.score >= 72.0
    ...
    if route == "ADAPTIVE_BREAKOUT":
        if symbol not in BREAKOUT_ENABLED:
            return False

        if symbol in {"BTCUSDT", "ETHUSDT"}:
            return signal.score >= 74.0
        ...
        for pullback in (
            ...
            candidates.append(
                ProfitCandidate(
                    ...
                    route="PULLBACK",
                    valid=pullback.valid,
                    score=pullback.score,
                    entry_price=(
                        pullback.entry_price
                    ),
                    stop_price=(
                        pullback.stop_price
                    ),
                    risk_percent=(
                        pullback.risk_percent
                    ),
                    projected_target=(
                        pullback.projected_target
                    ),
                    reward_risk=(
                        pullback.reward_risk
                    ),
                    failed_checks=(
...
def run_backtest() -> dict[str, Any]:
    print("=" * 126)
    print("FA CRYPTO ENGINE — V3E5 DEVELOPMENT EDGE-DISCOVERY BACKTEST")
    print("=" * 126)
    print(
    ...
    print()
    print("=" * 126)
    print("FA CRYPTO ENGINE — V3E5 DEVELOPMENT RESULT")
    print("=" * 126)
...
if __name__ == "__main__":
    try:
        ...
        print()
        print("V3E5 development backtest stopped manually.")

    except Exception as error:
        print()
        print("=" * 126)
        print("V3E5 DEVELOPMENT BACKTEST ERROR")
        print("=" * 126)
