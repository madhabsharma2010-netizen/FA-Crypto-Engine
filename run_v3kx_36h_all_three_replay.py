...
V3KV_EVENT_OUTPUT = Path(
    "reports/"
    f"v3kx_36h_all_three_observer_{TAG}_events.csv"
)

V3KV_DISPOSITION_OUTPUT = Path(
    "reports/"
    f"v3kx_36h_all_three_observer_{TAG}_dispositions.csv"
)

v3kx_36h_all_three_rows: list[
    dict[str, object]
] = []
...
def v3kv_record_due(
    ...
    **extra: object,
    ...
    v3kx_36h_all_three_rows.append(
        row
    )
...
TRADE_OUTPUT = Path(
    f"reports/"
    f"v3kx_36h_all_three_{TAG}_trades.csv"
)

EQUITY_OUTPUT = Path(
    f"reports/"
    f"v3kx_36h_all_three_{TAG}_equity.csv"
)

SUMMARY_OUTPUT = Path(
    f"reports/"
    f"v3kx_36h_all_three_{TAG}_summary.csv"
)

STATE_FILE = Path(
    f"state/backtests/"
    f"v3kx_36h_all_three_{TAG}_risk_guard.json"
)
...
for event_time in timeline:
    ...
    for symbol in sorted(
        ...
        if (
            completed_hours == 36
            and exit_now_net_r <= 0.0
            and mfe_gross_r < 0.5
            and stop_progress_r <= 0.0
            and symbol not in pending_exits
        ):
            pending_exits[symbol] = {
                "due_time": snapshot_time,
                "reason": "V3KX_36H_ALL_THREE",
            }
...
v3kx_36h_all_three_detail = pd.DataFrame(
    v3kx_36h_all_three_rows
)

v3kx_36h_all_three_detail.to_csv(
    V3KV_EVENT_OUTPUT,
    index=False,
)

if v3kx_36h_all_three_detail.empty:
...
print(
    "Observer rows                 : "
    f"{len(v3kx_36h_all_three_detail)}"
)
...
if not v3kx_36h_all_three_detail.empty:
    ...
    missing_columns = sorted(
        ...
        - set(
            v3kx_36h_all_three_detail.columns
        )
    )
    ...
    duplicate_keys = int(
        v3kx_36h_all_three_detail.duplicated(
            subset=[
                "snapshot_key",
            ]
        ).sum()
