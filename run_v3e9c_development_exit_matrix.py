...
def build_complete_offline_cache(module):
    ...
    # Defensive patch: any later reference through the diagnostics
    # module also stays offline.
    module.v3d_diagnostics.load_frame = (
        offline_load_frame
    )
...
if __name__ == "__main__":
    try:
        run_matrix(
            ...
            report_file=Path(
                "reports/"
                "v3e9c_development_exit_matrix.csv"
            ),
        )
        ...
        print()
        print(
            "V3E9C development matrix stopped manually."
        )
        ...
        print("=" * 138)
        print(
            "V3E9C DEVELOPMENT MATRIX ERROR"
        )
        print("=" * 138)
        print(
            f"{type(error).__name__}: {error}"
