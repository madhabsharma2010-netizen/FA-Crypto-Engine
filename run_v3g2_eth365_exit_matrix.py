...
# ============================================================
# V3G2 EXIT POLICY ? MUST LOAD BEFORE OUTPUT PATH CONSTANTS
# ============================================================

V3G2_EXIT_POLICY = os.environ.get(
    "V3G2_EXIT_POLICY",
    "BASELINE",
).upper()

VALID_V3G2_EXIT_POLICIES = {
    "BASELINE",
    "EMA10",
    "EMA20",
}

if V3G2_EXIT_POLICY not in VALID_V3G2_EXIT_POLICIES:
    raise ValueError(
        "Invalid V3G2_EXIT_POLICY: "
        f"{V3G2_EXIT_POLICY}"
    )

V3G2_OUTPUT_TAG = V3G2_EXIT_POLICY.lower()
...
v3d_diagnostics.BACKTEST_START = DEVELOPMENT_START
v3d_diagnostics.BACKTEST_END = DEVELOPMENT_END
...
BACKTEST_START = DEVELOPMENT_START
BACKTEST_END = DEVELOPMENT_END
...
# ============================================================================
# V3E6 SELECTIVE DEVELOPMENT — V3D3 RISK/EXIT ENGINE FROZEN
# ============================================================================
...
TRADE_HISTORY_FILE = Path(
    f"logs/backtests/v3g2_eth365_{V3G2_OUTPUT_TAG}_trades.csv"
)
EVENT_HISTORY_FILE = Path(
    f"logs/backtests/v3g2_eth365_{V3G2_OUTPUT_TAG}_events.csv"
)
SUMMARY_FILE = Path(
    f"reports/v3g2_eth365_{V3G2_OUTPUT_TAG}_summary.csv"
)
SYMBOL_SUMMARY_FILE = Path(
    f"reports/v3g2_eth365_{V3G2_OUTPUT_TAG}_symbols.csv"
)
...
# V3G2 experimental route.
# Signal timestamps/levels come from the already-audited V3G1
# no-lookahead 365-day-high diagnostic.
ETH365_ROUTE = "ETH_365D"
ETH365_SIGNAL_FILE = Path(
...
def inject_eth365_candidates(
    ...
    """
    Inject the V3G1 no-lookahead ETH 365D breakout signals.

    IMPORTANT:
    - 365D level is NOT recalculated from the short V3E6 warmup.
    - Initial stop reuses the existing V3E6 breakout stop structure.
    - No new leverage/risk rule is introduced.
    """

    if not ETH365_SIGNAL_FILE.exists():
        raise FileNotFoundError(
            f"Missing V3G1 signal file: "
            f"{ETH365_SIGNAL_FILE}"
        )
    ...
    if missing:
        raise ValueError(
            "V3G1 signal file missing columns: "
            f"{sorted(missing)}"
        )
    ...
    print(
        "V3G2 ETH365 valid signals injected: "
        f"{injected}"
    )
...
def run_backtest() -> dict[str, Any]:
    print("=" * 126)
    print("FA CRYPTO ENGINE — V3G2 ETH365 BASELINE EXIT DIAGNOSTIC")
    print("=" * 126)
    print(
        f"Development window: {BACKTEST_START} to {BACKTEST_END}"
    )
    print(
        "Risk core: V3D3 frozen | "
        f"ETH365 exit policy: {V3G2_EXIT_POLICY}"
    )
    ...
    for event_time in common_15m:
        ...
        if V3G2_EXIT_POLICY in {"EMA10", "EMA20"}:
    ...
    print()
    print("=" * 126)
    print("FA CRYPTO ENGINE — V3G2 ETH365 BASELINE RESULT")
    print("=" * 126)
