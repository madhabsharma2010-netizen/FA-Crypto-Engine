...
V3KV_EVENT_OUTPUT = Path(
    "reports/"
    f"v3kv_opportunity_observer_{TAG}_events.csv"
)

V3KV_DISPOSITION_OUTPUT = Path(
    "reports/"
    f"v3kv_opportunity_observer_{TAG}_dispositions.csv"
)

v3kv_observer_rows: list[
    dict[str, object]
] = []
...
EQUITY_OUTPUT = Path(
    f"reports/"
    f"v3kv_observer_{TAG}_equity.csv"
)

SUMMARY_OUTPUT = Path(
    f"reports/"
    f"v3kv_observer_{TAG}_summary.csv"
)

STATE_FILE = Path(
    f"state/backtests/"
    f"v3kv_observer_{TAG}_risk_guard.json"
)
...
v3kv_observer_detail = pd.DataFrame(
    v3kv_observer_rows
)

v3kv_observer_detail.to_csv(
    V3KV_EVENT_OUTPUT,
    index=False,
)

if v3kv_observer_detail.empty:
