...
# ============================================================================
# V3KU ISOLATED REPLAY MODE
# ============================================================================

V3KU_MODE = os.environ.get(
    "V3KU_MODE",
    "BASELINE",
).strip().upper()
...
if V3KU_MODE not in V3KU_VALID_MODES:
    raise ValueError(
        "Unsupported V3KU mode: "
        f"{V3KU_MODE}"
    )

V3KU_BLOCK_CANDIDATE = (
    V3KU_MODE
    == "BLOCK_BUILD_FRESH_BROAD_RECOVERY"
)
...
V3KU_MODE_TAG = (
    "baseline"
    if V3KU_MODE == "BASELINE"
    else "block_build_fresh_broad_recovery"
)

RUN_TAG = (
    f"{TAG}_{V3KU_MODE_TAG}"
)

V3KU_CANDIDATE_SOURCE = Path(
    "reports/"
    "v3ks_active_market_snapshot.csv"
)

V3KU_EXPECTED_CANDIDATE_ROWS = {
    "2022": 770,
    "2023": 1185,
    "2024": 1054,
    "2025H1": 499,
...
def load_v3ku_block_keys() -> set[
    tuple[
        str,
        pd.Timestamp,
    ]
]:
    if not V3KU_BLOCK_CANDIDATE:
        return set()

    if not V3KU_CANDIDATE_SOURCE.exists():
        raise FileNotFoundError(
            "MISSING_V3KU_CANDIDATE_SOURCE: "
            f"{V3KU_CANDIDATE_SOURCE}"
        )
    ...
    if missing_columns:
        raise RuntimeError(
            "V3KU_CANDIDATE_COLUMNS_MISSING: "
            f"{missing_columns}"
        )
    ...
    candidate = candidate_data.loc[
        candidate_data["window"].eq(
            WINDOW
        )
        & candidate_data["band"].eq(
            "BUILD"
        )
        & candidate_data[
            "path_state"
...
V3KU_BLOCK_KEYS = (
    load_v3ku_block_keys()
)

V3KU_BLOCKED_SIGNALS = 0
...
TRADE_OUTPUT = Path(
    f"reports/"
    f"v3ku_shared_portfolio_{RUN_TAG}_trades.csv"
)

EQUITY_OUTPUT = Path(
    f"reports/"
    f"v3ku_shared_portfolio_{RUN_TAG}_equity.csv"
)

SUMMARY_OUTPUT = Path(
    f"reports/"
    f"v3ku_shared_portfolio_{RUN_TAG}_summary.csv"
)

STATE_FILE = Path(
    f"state/backtests/"
    f"v3ku_shared_portfolio_{RUN_TAG}_risk_guard.json"
)
...
print("=" * 112)
# ============================================================================
# V3KU REPLAY METADATA
# ============================================================================

V3KU_METADATA_OUTPUT = Path(
    "reports/"
    f"v3ku_shared_portfolio_{RUN_TAG}_metadata.csv"
)

V3KU_METADATA_OUTPUT.parent.mkdir(
    parents=True,
    exist_ok=True,
)
    ...
    [
        {
            "window": WINDOW,
            "mode": V3KU_MODE,
            "candidate_key_count": len(
                V3KU_BLOCK_KEYS
            ),
            "blocked_signal_count": (
                V3KU_BLOCKED_SIGNALS
            ),
            "trade_output": str(
                TRADE_OUTPUT
            ),
...
print(
    "V3KU candidate keys           : "
    f"{len(V3KU_BLOCK_KEYS)}"
)

print(
    "V3KU blocked portfolio signals: "
    f"{V3KU_BLOCKED_SIGNALS}"
)

print(
    "V3KU metadata report          : "
    f"{V3KU_METADATA_OUTPUT}"
)
