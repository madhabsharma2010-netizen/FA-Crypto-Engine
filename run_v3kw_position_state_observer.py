...
V3KV_EVENT_OUTPUT = Path(
    "reports/"
    f"v3kw_position_state_observer_{TAG}_events.csv"
)

V3KV_DISPOSITION_OUTPUT = Path(
    "reports/"
    f"v3kw_position_state_observer_{TAG}_dispositions.csv"
)

v3kw_state_observer_rows: list[
    dict[str, object]
] = []
...
def v3kv_record_due(
    ...
    **extra: object,
    ...
    v3kw_state_observer_rows.append(
        row
    )
...
TRADE_OUTPUT = Path(
    f"reports/"
    f"v3kw_state_observer_{TAG}_trades.csv"
)

EQUITY_OUTPUT = Path(
    f"reports/"
    f"v3kw_state_observer_{TAG}_equity.csv"
)

SUMMARY_OUTPUT = Path(
    f"reports/"
    f"v3kw_state_observer_{TAG}_summary.csv"
)

STATE_FILE = Path(
    f"state/backtests/"
    f"v3kw_state_observer_{TAG}_risk_guard.json"
)
...
for event_time in timeline:
    ...
    for candidate_rank, candidate in enumerate(
        ...
        if (
            ...
            incoming_breakout_strength = float(
                candidate[
                    "breakout_strength"
                ]
            )
            ...
            for (
                ...
                existing_entry_time = pd.Timestamp(
                    existing_position[
                        "entry_time"
                    ]
                )
                ...
                existing_initial_stop = float(
                    existing_position[
                        "initial_stop"
                    ]
                )

                existing_stop_price = float(
                    existing_position[
                        "stop_price"
                    ]
                )

                existing_initial_risk_per_unit = float(
                    existing_position[
                        "initial_risk_per_unit"
                    ]
                )

                existing_initial_risk_eur = float(
                    existing_position[
                        "initial_risk_eur"
                    ]
                )
                ...
                existing_exit_now_net_r = (
                    existing_exit_now_net_pnl
                    / existing_initial_risk_eur
                    if existing_initial_risk_eur
                    > 0
                    else None
                )
                ...
                existing_gross_unrealized_r = (
                    ...
                    / existing_initial_risk_per_unit
                    if existing_initial_risk_per_unit
                    > 0
                    else None
                )
                ...
                existing_breakout_strength = float(
                    existing_position[
                        "breakout_strength"
                    ]
                )

                v3kw_position_snapshots.append(
                    {
                        "window":
                            WINDOW,
...
v3kw_state_observer_detail = pd.DataFrame(
    v3kw_state_observer_rows
)

v3kw_state_observer_detail.to_csv(
    V3KV_EVENT_OUTPUT,
    index=False,
)

if v3kw_state_observer_detail.empty:
...
print(
    "Observer rows                 : "
    f"{len(v3kw_state_observer_detail)}"
)
...
# ============================================================================
# V3KW POSITION-STATE SNAPSHOT OUTPUT
# ============================================================================

v3kw_snapshot_detail = pd.DataFrame(
    v3kw_position_snapshots
)

expected_snapshot_events = int(
    counters[
        "skipped_max_positions"
    ]
)
...
if len(
    v3kw_snapshot_detail
) != expected_snapshot_rows:

    raise RuntimeError(
        "V3KW_SNAPSHOT_ROW_COUNT_MISMATCH: "
        f"events={expected_snapshot_events}, "
        f"positions={MAX_OPEN_POSITIONS}, "
        f"expected_rows="
        f"{expected_snapshot_rows}, "
        f"actual_rows="
        f"{len(v3kw_snapshot_detail)}"
    )


if not v3kw_snapshot_detail.empty:

    duplicate_snapshot_keys = int(
        v3kw_snapshot_detail.duplicated(
            subset=[
                "window",
                "snapshot_key",
            ]
    ...
    if duplicate_snapshot_keys != 0:
        raise RuntimeError(
            "V3KW_DUPLICATE_SNAPSHOT_KEYS: "
            f"{duplicate_snapshot_keys}"
        )

    invalid_position_counts = int(
        (
            v3kw_snapshot_detail[
                "position_count"
            ]
    ...
    if invalid_position_counts != 0:
        raise RuntimeError(
            "V3KW_INVALID_POSITION_COUNTS: "
            f"{invalid_position_counts}"
        )
    ...
    if not null_counts.empty:
        raise RuntimeError(
            "V3KW_REQUIRED_SNAPSHOT_NULLS: "
            f"{null_counts.to_dict()}"
        )


v3kw_snapshot_output = (
    Path("reports")
    / (
        "v3kw_position_state_snapshots_"
        f"{TAG}.csv"
    )
)

v3kw_snapshot_detail.to_csv(
    v3kw_snapshot_output,
    index=False,
)
...
print()
print("=" * 112)
print("V3KW POSITION-STATE OBSERVER")
print("=" * 112)
print(
    f"MAX_POSITIONS events         : "
    f"{expected_snapshot_events}"
)
print(
    f"Position snapshot rows       : "
    f"{len(v3kw_snapshot_detail)}"
)
print(
    f"Rows per event               : "
