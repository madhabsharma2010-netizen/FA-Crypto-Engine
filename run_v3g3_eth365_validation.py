...
# ============================================================
# V3G3 EXIT POLICY ? MUST LOAD BEFORE OUTPUT PATH CONSTANTS
# ============================================================

V3G3_EXIT_POLICY = os.environ.get(
    "V3G3_EXIT_POLICY",
    "BASELINE",
).upper()

VALID_V3G3_EXIT_POLICIES = {
    "BASELINE",
    "EMA10",
}

if V3G3_EXIT_POLICY not in VALID_V3G3_EXIT_POLICIES:
    raise ValueError(
        "Invalid V3G3_EXIT_POLICY: "
        f"{V3G3_EXIT_POLICY}"
    )

V3G3_OUTPUT_TAG = V3G3_EXIT_POLICY.lower()
...
VALIDATION_END = pd.Timestamp("2026-07-01 00:00:00")
...
v3d_diagnostics.BACKTEST_START = VALIDATION_START
v3d_diagnostics.BACKTEST_END = VALIDATION_END
...
BACKTEST_START = VALIDATION_START
BACKTEST_END = VALIDATION_END
...
TRADE_HISTORY_FILE = Path(
    f"logs/backtests/v3g3_eth365_validation_{V3G3_OUTPUT_TAG}_trades.csv"
)
EVENT_HISTORY_FILE = Path(
    f"logs/backtests/v3g3_eth365_validation_{V3G3_OUTPUT_TAG}_events.csv"
)
SUMMARY_FILE = Path(
    f"reports/v3g3_eth365_validation_{V3G3_OUTPUT_TAG}_summary.csv"
)
SYMBOL_SUMMARY_FILE = Path(
    f"reports/v3g3_eth365_validation_{V3G3_OUTPUT_TAG}_symbols.csv"
)
...
# V3G3 untouched-validation route.
...
def inject_eth365_candidates(
    ...
    raw = get_historical_candles(
        ...
        end_time=VALIDATION_END.strftime(
            "%Y-%m-%d %H:%M:%S"
        ),
    ...
    print(
        "V3G3 ETH365 valid signals injected: "
        f"{injected}"
    )
...
def run_backtest() -> dict[str, Any]:
    print("=" * 126)
    print("FA CRYPTO ENGINE â€” V3G2 ETH365 BASELINE EXIT DIAGNOSTIC")
    print("=" * 126)
    print(
        f"Validation window: {BACKTEST_START} to {BACKTEST_END}"
    )
    print(
        "Risk core: V3D3 frozen | "
        f"ETH365 exit policy: {V3G3_EXIT_POLICY}"
    )
    ...
    print()
    print("=" * 126)
    print("FA CRYPTO ENGINE â€” V3G2 ETH365 BASELINE RESULT")
    print("=" * 126)
