...
# Structural anti-churn safeguards.
ENTRY_CONFIRMATION_CANDLES = 3
SLEEP_EXIT_CONFIRMATION_CANDLES = 3
SELL_EXIT_CONFIRMATION_CANDLES = 2
REENTRY_COOLDOWN_CANDLES = 6

# Never open a token-sized filler position merely because
# a few euros of portfolio deployment room remain.
MIN_NOTIONAL_EUR = 500.0
MIN_TARGET_FILL_RATIO = 0.80

# DOGE is quarantined until it passes a separate high-beta
# validation. It remains in market breadth/ranking diagnostics.
DOGE_TRADING_ENABLED = False

ASSET_ALLOCATION_CAPS = {
    "BTCUSDT": 50.0,
    "ETHUSDT": 40.0,
    "SOLUSDT": 30.0,
    "XRPUSDT": 30.0,
...
TRADE_HISTORY_FILE = Path(
    "logs/backtests/v3c_portfolio_trade_history.csv"
)

SUMMARY_FILE = Path(
    "reports/v3c_portfolio_backtest_summary.csv"
)

SYMBOL_SUMMARY_FILE = Path(
    "reports/v3c_portfolio_symbol_summary.csv"
)
...
def allocate_ranked_assets(
    ...
    """
    Select at most two confirmed positive-trend leaders.

    Structural rules:
        - DOGE remains quarantined from trading.
        - High-beta assets cannot receive 50% allocation.
        - Maximum total portfolio deployment stays at 50%.
    """
    ...
    eligible = ranked[
        ...
        & (ranked["MOMENTUM_24H"] > 0.0)
        & (ranked["EMA50_SLOPE_12H"] > 0.0)
    ].copy()

    if not DOGE_TRADING_ENABLED:
        eligible = eligible[
    ...
    leader_cap = ASSET_ALLOCATION_CAPS.get(
        leader_symbol,
        30.0,
    )
    ...
    if len(eligible) == 1:
        desired = (
            50.0
            if (
                float(leader["RELATIVE_SCORE"]) >= 85.0
                and float(leader["REGIME_SCORE"]) >= 85.0
            )
            else 30.0
        )
        ...
        return (
            {leader_symbol: allocation}
            if allocation > 0
            else {}
        )

    second = eligible.iloc[1]
    second_symbol = str(second["symbol"])
    second_cap = ASSET_ALLOCATION_CAPS.get(
        second_symbol,
        30.0,
    )
    ...
    if (
        float(leader["RELATIVE_SCORE"]) >= 88.0
    ...
    if (
        float(second["RELATIVE_SCORE"]) >= 65.0
        and relative_gap <= 15.0
    ):
        leader_allocation = min(
            30.0,
            leader_cap,
        )
        second_allocation = min(
            20.0,
            second_cap,
        )
        ...
        if leader_allocation > 0:
            result[leader_symbol] = leader_allocation

        if second_allocation > 0:
            result[second_symbol] = second_allocation
...
def run_backtest() -> dict[str, Any]:
    print("=" * 118)
    print(
        "FA CRYPTO ENGINE — V3C CONFIRMED-REGIME PORTFOLIO BACKTEST"
    )
    print("=" * 118)
    ...
    def close_full_position(
        ...
        if exit_reason == "STOP LOSS":
            ...
            trailing_exits += 1
        elif exit_reason.startswith("REGIME SLEEP"):
            sleep_exits += 1
        elif exit_reason.startswith("STRATEGY SELL"):
            strategy_exits += 1
        ...
        # Prevent same-candle and rapid re-entry churn.
        cooldown_until_index[symbol] = (
            current_index
            + REENTRY_COOLDOWN_CANDLES
        )
        selection_streak[symbol] = 0
        sleep_streak[symbol] = 0
        sell_streak[symbol] = 0
    ...
    for current_index, candle_time in enumerate(
        common_times
    ):
        ...
        for symbol in SYMBOLS:
            if symbol in selected_symbols:
                selection_streak[symbol] += 1
                selected_counts[symbol] += 1
            else:
                selection_streak[symbol] = 0

            if (
                decision_by_symbol[symbol].regime
                == MarketRegime.SLEEP
            ):
                sleep_streak[symbol] += 1
            else:
                sleep_streak[symbol] = 0
        ...
        for symbol in list(
            ...
            if (
                sleep_streak[symbol]
                >= SLEEP_EXIT_CONFIRMATION_CANDLES
            ):
                close_full_position(
                    symbol,
                    close_price,
                    candle_time,
                    "REGIME SLEEP CONFIRMED",
                )
                continue

            if strategy_signal == "SELL":
                sell_streak[symbol] += 1
            else:
                sell_streak[symbol] = 0

            if (
                sell_streak[symbol]
                >= SELL_EXIT_CONFIRMATION_CANDLES
            ):
                close_full_position(
                    symbol,
                    close_price,
                    candle_time,
                    "STRATEGY SELL CONFIRMED",
                )
                continue
        ...
        qualified_market_assets = int(
            qualified_market_mask.sum()
        )

        leader_exceptional = bool(
            not ranked.empty
            and float(
                ranked.iloc[0]["RELATIVE_SCORE"]
            ) >= 90.0
            and float(
                ranked.iloc[0]["REGIME_SCORE"]
            ) >= 90.0
            and float(
                ranked.iloc[0]["MOMENTUM_24H"]
            ) > 0.5
        )

        market_breadth_open = (
            qualified_market_assets >= 2
            or leader_exceptional
        )
        ...
        if not block_new_entries:
            for symbol, allocation_percent in (
                ...
                if (
                    float(
                        ranked_row["MOMENTUM_24H"]
                    ) <= 0.0
                    or float(
                        ranked_row["EMA50_SLOPE_12H"]
                    ) <= 0.0
                ):
                    continue
                ...
                minimum_required_notional = max(
                    MIN_NOTIONAL_EUR,
                    target_notional
                    * MIN_TARGET_FILL_RATIO,
                )

                if (
                    notional <= 0
                    or notional
                    < minimum_required_notional
                ):
                    continue
    ...
    print("=" * 118)
    print(
        "FA CRYPTO ENGINE — V3C PORTFOLIO RESULT"
    )
    print("=" * 118)
...
if __name__ == "__main__":
    try:
        ...
        print()
        print(
            "V3C portfolio backtest stopped manually."
        )
